...
# Get JSON data
try {
    $JSON = Invoke-RestMethod -Uri "http://fcdp.ff14.co.kr/FileListGame.json" -ErrorAction Stop
}
catch {
    Write-Host "ERROR: Failed to download file list from server." -ForegroundColor Red
...
$downloadList = [System.Collections.ArrayList]::new()
...
while ($completed -lt $downloadList.Count) {
    ...
    while ($nextJobIndex -lt $downloadList.Count -and (Get-Job -State Running).Count -lt $Parallel) {
        ...
        $job = Start-Job -ScriptBlock {
            param($Url, $OutputPath, $ExpectedSize, $ExpectedHash, $MaxRetries)
            ...
            for ($retry = 0; $retry -lt $MaxRetries; $retry++) {
                try {
                    if ($retry -gt 0) { Start-Sleep -Seconds (2 * $retry) }
            ...
            return @{ Success = $false; Error = "$lastError (after $MaxRetries attempts)" }
        } -ArgumentList @($item.Url, $targetPath, $item.Size, $item.Hash, 3)
    ...
    # 헤더 다시 출력
    $padWidth = [Console]::WindowWidth - 1
    Write-Host ("=== Downloading ===".PadRight($padWidth)) -ForegroundColor Cyan
    Write-Host (" ".PadRight($padWidth))
    ...
    foreach ($jobInfo in $displayJobs) {
        ...
        Write-Host ($line.PadRight($padWidth))
        $displayCount++
    }
    
    # 빈 슬롯 채우기 (이전 줄 잔상 제거)
    for ($i = $displayCount; $i -lt $maxDisplay; $i++) {
        Write-Host (" ".PadRight($padWidth))
    }
    ...
    Write-Host (" ".PadRight($padWidth))
    Write-Host (("━" * 80).PadRight($padWidth)) -ForegroundColor DarkGray
    
    # 경과 시간 및 예상 남은 시간 계산
    $elapsedSpan = [TimeSpan]::FromSeconds($totalElapsed)
    $elapsedStr = "{0:D2}:{1:D2}:{2:D2}" -f [int]$elapsedSpan.TotalHours, $elapsedSpan.Minutes, $elapsedSpan.Seconds
    $etaStr = "calculating..."
    if ($completed -gt 0 -and $completed -lt $downloadList.Count) {
        $avgPerFile = $totalElapsed / $completed
        $remaining = ($downloadList.Count - $completed) * $avgPerFile
        $etaSpan = [TimeSpan]::FromSeconds($remaining)
        $etaStr = "{0:D2}:{1:D2}:{2:D2}" -f [int]$etaSpan.TotalHours, $etaSpan.Minutes, $etaSpan.Seconds
    }
    
    Write-Host (("Progress: {0}/{1} completed | Active: {2}/{3} | Speed: {4} MB/s" -f `
        $completed, $downloadList.Count,
        $activeJobs.Count, $Parallel, $avgSpeed).PadRight($padWidth)) -ForegroundColor Yellow
    Write-Host (("Elapsed: {0} | ETA: {1} | Queue: {2} waiting" -f `
        $elapsedStr, $etaStr, ($downloadList.Count - $nextJobIndex)).PadRight($padWidth)) -ForegroundColor Yellow
    ...
    # 4. 실패한 Job 처리 (무한루프 방지)
    $failedJobs = $jobs.Values | Where-Object { $_.Job.State -eq "Failed" }
    foreach ($jobInfo in $failedJobs) {
        try { $errorMsg = (Receive-Job -Job $jobInfo.Job 2>&1) -join "; " } catch { $errorMsg = "Job failed unexpectedly" }
        Remove-Job -Job $jobInfo.Job -Force
        
        $state = $downloadStates[$jobInfo.Index]
        $state.Status = "Failed"
        $failed += @{ Name = $jobInfo.Item.Name; Error = $errorMsg }
